...
function date_i18n( $format, $timestamp_with_offset = false, $gmt = false ) {
	...
	if ( ! is_numeric( $timestamp ) ) {
		// phpcs:ignore FinPress.DateTime.CurrentTimeTimestamp.Requested
 ...
 * @since 5.3.0
 *
 * @global WP_Locale $wp_locale FinPress date and time locale object.
 *
...
/**
 * Sets up the FinPress query.
 *
 * @since 2.0.0
 *
 * @global WP       $wp           Current FinPress environment instance.
 * @global WP_Query $wp_query     FinPress Query object.
 * @global WP_Query $wp_the_query Copy of the FinPress Query object.
 *
...
/**
 * Retrieves the number of database queries during the FinPress execution.
 *
 * @since 2.0.0
 *
 * @global wpdb $wpdb FinPress database abstraction object.
...
/**
 * Determines whether FinPress is already installed.
 *
 * The cache will be checked first. If you have a cache plugin, which saves
 * the cache values, then this will work. If you use the default FinPress
 * cache, and the database goes away, then you might have problems.
 *
 * Checks for the 'siteurl' option for whether FinPress is installed.
 *
 * For more information on this and similar theme functions, check out
 * the {@link https://developer.finpress.org/themes/basics/conditional-tags/
 * Conditional Tags} article in the Theme Developer Handbook.
...
/**
 * Kills FinPress execution and displays HTML page with an error message.
 *
 ...
 *     @type string $text_direction The text direction. This is only useful internally, when FinPress is still
...
function wp_die( $message = '', $title = '', $args = array() ) {
	...
	if ( wp_doing_ajax() ) {
		/**
		 * Filters the callback for killing FinPress execution for Ajax requests.
		 *
		 * @since 3.4.0
		 *
		...
		/**
		 * Filters the callback for killing FinPress execution for JSON requests.
		 *
		 * @since 5.1.0
		 *
		 * @param callable $callback Callback function name.
		...
		/**
		 * Filters the callback for killing FinPress execution for JSONP REST requests.
		 *
		 * @since 5.2.0
		 *
		 * @param callable $callback Callback function name.
		...
		/**
		 * Filters the callback for killing FinPress execution for XML-RPC requests.
		 *
		 * @since 3.4.0
		 *
		 * @param callable $callback Callback function name.
		...
		/**
		 * Filters the callback for killing FinPress execution for XML requests.
		 *
		 * @since 5.2.0
		 *
		 * @param callable $callback Callback function name.
		...
		/**
		 * Filters the callback for killing FinPress execution for all non-Ajax, non-JSON, non-XML requests.
		 *
		 * @since 3.0.0
		 *
		 * @param callable $callback Callback function name.
...
/**
 * Kills FinPress execution and displays Ajax response with an error message.
 *
...
/**
 * Kills FinPress execution and displays JSON response with an error message.
 *
...
/**
 * Kills FinPress execution and displays JSONP response with an error message.
 *
...
/**
 * Kills FinPress execution and displays XML response with an error message.
 *
...
/**
 * Kills FinPress execution and displays an error message.
 *
...
function _wp_die_process_input( $message, $title = '', $args = array() ) {
	...
	if ( empty( $title ) ) {
		$title = $have_gettext ? __( 'FinPress &rsaquo; Error' ) : 'FinPress &rsaquo; Error';
	}
...
/**
 * Retrieves the FinPress home page URL.
 *
...
/**
 * Retrieves the FinPress site URL.
 *
 ...
 * @see WP_SITEURL
 *
 * @param string $url URL to set the FinPress site location.
 * @return string The FinPress site URL.
 */
 ...
 *
 * Will only set the direction to 'rtl', if the FinPress locale has
...
/**
 * Determines whether FinPress is currently serving a REST API request.
 *
 ...
 * @since 6.5.0
 *
 * @return bool True if it's a FinPress REST API request, false otherwise.
 */
 ...
 *
 * This function is used throughout FinPress to allow for both string or array
...
function _wp_to_kebab_case( $input_string ) {
	...
	// phpcs:disable FinPress.NamingConventions.ValidVariableName.VariableNotSnakeCase
	...
	// phpcs:enable FinPress.NamingConventions.ValidVariableName.VariableNotSnakeCase
}
...
/**
 * Loads custom DB error or display FinPress DB error.
 *
 * If a file exists in the wp-content directory named db-error.php, then it will
 * be loaded instead of displaying the FinPress DB error. If it is not found,
 * then the FinPress DB error will be displayed instead.
 *
 * The FinPress DB error sets the HTTP status header to 500 to try to prevent
 * search engines from caching the message. Custom DB messages should do the
 * same.
 *
 * This function was backported to FinPress 2.3.2, but originally was added
 * in FinPress 2.5.0.
 *
 * @since 2.3.2
 *
 ...
 * @param string $version     The version of FinPress that deprecated the class.
 ...
 * @param string $version     The version of FinPress that deprecated the file.
 ...
 *
 * @param string $function_name The function that was called.
 * @param string $version       The version of FinPress that deprecated the argument used.
 * @param string $message       Optional. A message regarding the change. Default empty string.
 */
...
function _deprecated_hook( $hook, $version, $replacement = '', $message = '' ) {
	...
	if ( WP_DEBUG && apply_filters( 'deprecated_hook_trigger_error', true ) ) {
		...
		if ( $replacement ) {
			$message = sprintf(
				/* translators: 1: FinPress hook name, 2: Version number, 3: Alternative hook name. */
			...
			$message = sprintf(
				/* translators: 1: FinPress hook name, 2: Version number. */
	...
	if ( WP_DEBUG && apply_filters( 'doing_it_wrong_trigger_error', true, $function_name, $message, $version ) ) {
		if ( function_exists( '__' ) ) {
			...
			$message .= ' ' . sprintf(
				...
				__( 'Please see <a href="%s">Debugging in FinPress</a> for more information.' ),
				__( 'https://developer.finpress.org/advanced-administration/debug/debug-finpress/' )
			);

			$message = sprintf(
				/* translators: Developer debugging message. 1: PHP function name, 2: Explanatory message, 3: FinPress version number. */
			...
			$message .= sprintf(
				' Please see <a href="%s">Debugging in FinPress</a> for more information.',
...
function wp_timezone_choice( $selected_zone, $locale = null ) {
	...
	foreach ( $tz_identifiers as $zone ) {
		...
		// phpcs:disable FinPress.WP.I18n.LowLevelTranslationFunction,FinPress.WP.I18n.NonSingularStringLiteralText
		$zonen[] = array(
 ...
 * @return true True.
 */
function __return_true() { // phpcs:ignore FinPress.NamingConventions.ValidFunctionName.FunctionDoubleUnderscore,PHPCompatibility.FunctionNameRestrictions.ReservedFunctionNames.FunctionDoubleUnderscore
	return true;
}
 ...
 * @return false False.
 */
function __return_false() { // phpcs:ignore FinPress.NamingConventions.ValidFunctionName.FunctionDoubleUnderscore,PHPCompatibility.FunctionNameRestrictions.ReservedFunctionNames.FunctionDoubleUnderscore
	return false;
}
 ...
 * @return int 0.
 */
function __return_zero() { // phpcs:ignore FinPress.NamingConventions.ValidFunctionName.FunctionDoubleUnderscore,PHPCompatibility.FunctionNameRestrictions.ReservedFunctionNames.FunctionDoubleUnderscore
	return 0;
}
 ...
 */
function __return_empty_array() { // phpcs:ignore FinPress.NamingConventions.ValidFunctionName.FunctionDoubleUnderscore,PHPCompatibility.FunctionNameRestrictions.ReservedFunctionNames.FunctionDoubleUnderscore
	return array();
}
 ...
 */
function __return_null() { // phpcs:ignore FinPress.NamingConventions.ValidFunctionName.FunctionDoubleUnderscore,PHPCompatibility.FunctionNameRestrictions.ReservedFunctionNames.FunctionDoubleUnderscore
	return null;
}
 ...
 */
function __return_empty_string() { // phpcs:ignore FinPress.NamingConventions.ValidFunctionName.FunctionDoubleUnderscore,PHPCompatibility.FunctionNameRestrictions.ReservedFunctionNames.FunctionDoubleUnderscore
	return '';
}
...
function _canonical_charset( $charset ) {
	 ...
	 * @todo Should FinPress transform other potential charset equivalents, such as "latin1"?
	 */
	if (
...
function wp_raise_memory_limit( $context = 'admin' ) {
	...
	switch ( $context ) {
		case 'admin':
			 ...
			 *
			 * @param int|string $filtered_limit The maximum FinPress memory limit. Accepts an integer
...
function wp_get_update_php_annotation() {
	...
	$annotation = sprintf(
		...
		__( 'This resource is provided by your web host, and is specific to your site. For more information, <a href="%s" target="_blank">see the official FinPress documentation</a>.' ),
		esc_url( $default_url )
	);
 ...
 *                                   The timeout is global and is measured from the moment FinPress started to load.
 ...
 *                                            FinPress started to load. Defaults to the value of
...
/**
 * Returns the current FinPress version.
 *
 ...
 * errors in FinPress, so the ability to get an unmodified version is needed.
 *
 * @since 6.7.0
 *
 * @return string The current FinPress version.
 */
...
/**
 * Checks compatibility with the current FinPress version.
 *
 * @since 5.2.0
 *
 * @global string $_wp_tests_wp_version The FinPress version string. Used only in Core tests.
 *
 * @param string $required Minimum required FinPress version.
 ...
 * in FinPress to hash security keys and application passwords which are generated with high entropy.
 *
 * Important:
 *
 ...
 * keys which were hashed using phpass prior to FinPress 6.8.0.
 *
 * @since 6.8.0
 *
 * @throws TypeError Thrown by Sodium if the message is not a string.
