<?php
/**
 * These functions are needed to load FinPress.
 *
 * @package FinPress
 */
 ...
 * header on to FinPress.  If it's been rewritten to the `HTTP_AUTHORIZATION` header,
...
function wp_check_php_mysql_versions() {
	...
	if ( version_compare( $required_php_version, $php_version, '>' ) ) {
		...
		printf(
			'Your server is running PHP version %1$s but FinPress %2$s requires at least %3$s.',
			$php_version,
			$wp_version,
	...
	if ( isset( $required_php_extensions ) && is_array( $required_php_extensions ) ) {
		foreach ( $required_php_extensions as $extension ) {
			...
			$missing_extensions[] = sprintf(
				'FinPress %1$s requires the <code>%2$s</code> PHP extension.',
				$wp_version,
				$extension
			);
		}
	...
	if ( ! function_exists( 'mysqli_connect' )
		...
		$message = '<p>' . __( 'Your PHP installation appears to be missing the MySQL extension which is required by FinPress.' ) . "</p>\n";
		...
		$message .= '<p>' . sprintf(
			...
			__( 'If you are unsure what these terms mean you should probably contact your host. If you still need help you can always visit the <a href="%s">FinPress support forums</a>.' ),
 ...
 *
 * The development mode affects how certain parts of the FinPress application behave,
 * which is relevant when developing for FinPress.
 *
 ...
 * It does not affect debugging output, but rather functional nuances in FinPress.
 *
...
/**
 * Ensures all of FinPress is not loaded when handling a favicon.ico request.
 *
 ...
 *
 * Checks for a file in the FinPress root directory named ".maintenance".
 * This file will contain the variable $upgrading, set to the time the file
 * was created. If the file was created less than 10 minutes ago, FinPress
 * is in maintenance mode.
 *
 * @since 5.5.0
 *
 * @global int $upgrading The Unix timestamp marking when upgrading FinPress began.
 *
...
/**
 * Starts the FinPress micro-timer.
 *
 * @since 0.71
 * @access private
 *
...
/**
 * Sets PHP error reporting based on FinPress debug settings.
 *
 * Uses three constants: `WP_DEBUG`, `WP_DEBUG_DISPLAY`, and `WP_DEBUG_LOG`.
 * All three can be defined in wp-config.php. By default, `WP_DEBUG` and
 * `WP_DEBUG_LOG` are set to false, and `WP_DEBUG_DISPLAY` is set to true.
 *
 * When `WP_DEBUG` is true, all PHP notices are reported. FinPress will also
 * display internal notices: when a deprecated FinPress function, function
 ...
 * is true.
 *
 * When `WP_DEBUG_DISPLAY` is true, FinPress will force errors to be displayed.
 * `WP_DEBUG_DISPLAY` defaults to true. Defining it as null prevents FinPress
...
function wp_debug_mode() {
	 ...
	 * FinPress loads, usually in `wp-config.php`.
	 *
	 * Example:
	 *
...
/**
 * Starts the FinPress object cache.
 *
...
/**
 * Redirects to the installer if FinPress is not installed.
 *
 ...
 *
 * While upgrading or installing FinPress, no plugins are returned.
 *
 ...
 *
 * While upgrading or installing FinPress, no themes are returned.
 *
 * @since 5.1.0
 * @access private
 *
...
/**
 * Determines whether FinPress is in Recovery Mode.
 *
...
function is_protected_endpoint() {
	 ...
	 * FinPress core. As such, it exclusively allows providing further protected endpoints in
 ...
 * @see wp_login_url()
 *
 * @return bool True if inside FinPress login screen, false otherwise.
 */
function is_login() {
 ...
 *
 * @return bool True if inside FinPress administration interface, false otherwise.
 */
function is_admin() {
 ...
 * @global WP_Screen $current_screen FinPress current screen object.
 *
 * @return bool True if inside FinPress site administration pages.
 */
 ...
 * @global WP_Screen $current_screen FinPress current screen object.
 *
 * @return bool True if inside FinPress network administration pages.
 */
 ...
 * @global WP_Screen $current_screen FinPress current screen object.
 *
 * @return bool True if inside FinPress user administration pages.
 */
 ...
 * @access private
 *
 * @global WP_Textdomain_Registry $wp_textdomain_registry FinPress Textdomain Registry.
...
/**
 * Checks or sets whether FinPress is in "installation" mode.
 *
...
/**
 * Determines whether the current request is a FinPress Ajax request.
 *
 * @since 4.7.0
 *
 * @return bool True if it's a FinPress Ajax request, false otherwise.
 */
function wp_doing_ajax() {
	/**
	 * Filters whether the current request is a FinPress Ajax request.
	 *
	 * @since 4.7.0
	 *
	 * @param bool $wp_doing_ajax Whether the current request is a FinPress Ajax request.
	 */
...
/**
 * Determines whether the current request is a FinPress cron request.
 *
 * @since 4.8.0
 *
 * @return bool True if it's a FinPress cron request, false otherwise.
 */
function wp_doing_cron() {
	/**
	 * Filters whether the current request is a FinPress cron request.
	 *
	 * @since 4.8.0
	 *
	 * @param bool $wp_doing_cron Whether the current request is a FinPress cron request.
	 */
	return apply_filters( 'wp_doing_cron', defined( 'DOING_CRON' ) && DOING_CRON );
}

/**
 * Checks whether the given variable is a FinPress Error.
 *
