...
function wp_enqueue_media( $args = array() ) {
	 ...
	 *
	 * By default, if this filter does not return an array, the function
	 * `get_media_library_months_with_files()` wil run a query to determine
	 * the months that have media items. This query can be expensive for large
	 * media libraries, so it may be desirable for sites to override this
	 * behavior for example by using a transient.
	 *
	 * @since 4.7.4
	 *
	 * @link https://core.trac.wordpress.org/ticket/31071
 ...
 * Fires right after `media_library_months_with_files` if not overridden by it.
 * Use this function if you intend to store it's result in a transient or similar.
 *
 * @since 6.9.0
 *
 * @global wpdb $wpdb WordPress database abstraction object.
 *
 * @return array Array of objects with `month` and `year` properties.
 */
