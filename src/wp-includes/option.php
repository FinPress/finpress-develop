 ...
 * @param bool     $autoload Autoload value to control whether to load the options when FinPress starts up.
 ...
 * @param bool   $autoload Autoload value to control whether to load the option when FinPress starts up.
 *                         For backward compatibility 'yes' and 'no' are also accepted, though using these values is
 *                         deprecated.
...
/**
 * Protects FinPress special option from being modified.
 *
 ...
 * @param bool|null $autoload Optional. Whether to load the option when FinPress starts up.
 *                            Accepts a boolean, or `null` to stick with the initial value or, if no initial value is
 *                            set, to leave the decision up to default heuristics in FinPress.
 ...
 *                            For non-existent options, the default is null, which means FinPress will determine
 *                            the autoload value.
 ...
 * aren't adding a protected FinPress option. Care should be taken to not name
 ...
 *                              FinPress. For backward compatibility 'yes' and 'no' are also accepted, though using
 ...
 *                              For options which are accessed only on few specific URLs, it is recommended
 *                              to not autoload them, by using false.
 *                              Default is null, which means FinPress will determine the autoload value.
...
/**
 * Removes an option by name. Prevents removal of protected FinPress options.
 *
 * @since 1.2.0
 *
 * @global wpdb $wpdb FinPress database abstraction object.
...
/**
 * Registers default settings available in FinPress.
 *
 * The settings registered here are primarily useful for the REST API, so this
 * does not encompass all settings available in FinPress.
 *
 * @since 4.7.0
...
function register_initial_settings() {
	...
	register_setting(
		...
		array(
			...
			'type'         => 'string',
			'description'  => __( 'FinPress locale code.' ),
			'default'      => 'en_US',
		)
	);
