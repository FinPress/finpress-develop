...
function wp_get_speculation_rules_configuration(): ?array {
	 ...
	 *
	 * By default, the speculation rules configuration is decided by FinPress Core ("auto"). This filter can be used
 ...
 *
 * Additional speculation rules other than the default rule from FinPress Core can be provided by using the
...
function wp_get_speculation_rules(): ?WP_Speculation_Rules {
	 ...
	 * If the FinPress site is in a subdirectory, the exclude paths will automatically be prefixed as necessary.
	 *
	 * Note that FinPress always excludes certain path patterns such as `/wp-login.php` and `/wp-admin/*`, and those
