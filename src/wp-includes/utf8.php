 ...
 *     'Pi�a' === _wp_crawl_utf8( $pineapple, 'substitute' );
 *     null   === _wp_crawl_utf8( $pineapple, 'fail' );
 *
 ...
 */
function _wp_scan_utf8( string $bytes, int &$at, int &$invalid_length ): int {
	$end            = strlen( $bytes );
	$count          = 0;
	$invalid_length = 0;
...
function _wp_is_valid_utf8_fallback( string $bytes ): bool {
	...
	return $end === $at && 0 === $invalid_length;
}
 ...
 * Example:
 *
 *     $text = "text\x90wp\E2\x80\xC0test";
 *
 *     $chunks = iterator_to_array( wp_utf8_chunks( $text ) );
 *     array( 4, -1, 2, -2, -1, 4 ) === $chunks;
 *
 *     $at = 0;
 *     foreach ( wp_utf8_chunks( $text ) as $chunk_length ) {
 *         if ( $chunk_length >= 0 ) {
 *             echo substr( $text, $at, $chunk_length );
 *         } else {
 *             $bytes = substr( $text, $at, -$chunk_length );
 *             $bytes = implode( ' ', array_map( 'bin2hex', str_split( $bytes ) ) );
 *             echo "({$bytes})";
 *         }
 *
 *         $at += abs( $chunk_length );
 *     }
 *     // text(90)wp(e2 80)(c0)test
 *
 ...
 */
function wp_utf8_chunks( string $text ): Generator {
	$at             = 0;
	$was_at         = 0;
	$end            = strlen( $text );
	$invalid_length = 0;
