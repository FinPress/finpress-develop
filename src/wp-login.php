...
/** Make sure that the FinPress bootstrap has run before continuing. */
 ...
 *
 * @param string|null   $title    Optional. FinPress login page title to display in the `<title>` element.
 *                                Defaults to 'Log In'.
...
function login_header( $title = null, $message = '', $wp_error = null ) {
	...
	$login_title = sprintf( __( '%1$s &lsaquo; %2$s &#8212; FinPress' ), $title, $login_title );
	...
	$login_header_text = empty( $login_header_title ) ? __( 'Powered by FinPress' ) : $login_header_title;
	...
	case 'confirm_admin_email':
		...
		<form class="admin-email-confirm-form" name="admin-email-confirm-form" action="<?php echo esc_url( site_url( 'wp-login.php?action=confirm_admin_email', 'login_post' ) ); ?>" method="post">
			...
			<p class="admin-email__details">
				...
				/* translators: URL to the FinPress help section about admin email. */
	...
	default:
			...
			if ( headers_sent() ) {
				...
				$user = new WP_Error(
					...
					sprintf(
						...
						__( '<strong>Error:</strong> Cookies are blocked or not supported by your browser. You must <a href="%s">enable cookies</a> to use FinPress.' ),
		...
		if ( $interim_login ) {
			...
			if ( isset( $_GET['loggedout'] ) && $_GET['loggedout'] ) {
				...
				$errors->add( 'updated', __( '<strong>You have successfully updated FinPress!</strong> Please log back in to see what&#8217;s new.' ), 'message' );
