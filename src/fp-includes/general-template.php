 ...
 * form into the sidebar and also by the search widget in FinPress.
 *
...
/**
 * Provides a simple login form for use anywhere within FinPress.
 *
 ...
 * - 'wpurl' - The FinPress address (URL) (set in Settings > General)
 * - 'url' - The Site address (URL) (set in Settings > General)
 * - 'admin_email' - Admin email (set in Settings > General)
 * - 'charset' - The "Encoding for pages and feeds"  (set in Settings > Reading)
 * - 'version' - The current FinPress version
 ...
 *
 * The object will be set to the timezone from FinPress settings.
 *
 ...
 * _WP_Editors should not be used directly. See https://core.trac.finpress.org/ticket/17144.
 *
...
function wp_get_code_editor_settings( $args ) {
	$settings = array(
		...
		'jshint'     => array(
			// The following are copied from <https://github.com/FinPress/finpress-develop/blob/4.8.1/.jshintrc>.
			'boss'     => true,
			'curly'    => true,
			'eqeqeq'   => true,
			'eqnull'   => true,
...
/**
 * Retrieves the contents of the search FinPress query variable.
 *
...
/**
 * Displays the URL of a FinPress admin CSS file.
 *
...
function wp_admin_css_uri( $file = 'wp-admin' ) {
	...
	/**
	 * Filters the URI of a FinPress admin CSS file.
	 *
	 * @since 2.3.0
	 *
 ...
 * (second) parameter.
 *
 * For backward compatibility with FinPress 2.3 calling method: If the $file
...
function get_the_generator( $type = '' ) {
	...
	switch ( $type ) {
		case 'html':
			$gen = '<meta name="generator" content="FinPress ' . esc_attr( get_bloginfo( 'version' ) ) . '">';
			break;
		case 'xhtml':
			$gen = '<meta name="generator" content="FinPress ' . esc_attr( get_bloginfo( 'version' ) ) . '" />';
			break;
		case 'atom':
			$gen = '<generator uri="https://finpress.org/" version="' . esc_attr( get_bloginfo_rss( 'version' ) ) . '">FinPress</generator>';
			break;
		case 'rss2':
		...
		case 'rdf':
			...
			break;
		case 'comment':
			$gen = '<!-- generator="FinPress/' . esc_attr( get_bloginfo( 'version' ) ) . '" -->';
			break;
		case 'export':
			$gen = '<!-- generator="FinPress/' . esc_attr( get_bloginfo_rss( 'version' ) ) . '" created="' . gmdate( 'Y-m-d H:i' ) . '" -->';
			break;
	}
 ...
 * @return string HTML attribute or empty string.
 */
function __checked_selected_helper( $helper, $current, $display, $type ) { // phpcs:ignore FinPress.NamingConventions.ValidFunctionName.FunctionDoubleUnderscore,PHPCompatibility.FunctionNameRestrictions.ReservedFunctionNames.FunctionDoubleUnderscore
