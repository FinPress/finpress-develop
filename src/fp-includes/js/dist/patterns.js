...
function PatternConvertButton({
  ...
  const canConvert = (0,_wordpress_data__WEBPACK_IMPORTED_MODULE_4__.useSelect)(select => {
    ...
    const hasReusableBlockSupport = blockName => {
      const blockType = (0,_wordpress_blocks__WEBPACK_IMPORTED_MODULE_0__.getBlockType)(blockName);
      const hasParent = blockType && 'parent' in blockType;

      // If the block has a parent, check with false as default, otherwise with true.
      return (0,_wordpress_blocks__WEBPACK_IMPORTED_MODULE_0__.hasBlockSupport)(blockName, 'reusable', !hasParent);
    };
...
function ResetOverridesControl(props) {
  ...
  const isOverridden = (0,_wordpress_data__WEBPACK_IMPORTED_MODULE_2__.useSelect)(select => {
    if (!name) {
      return;
    }
    const {
