<?php
/**
 * FinPress database access abstraction class.
 *
 ...
 * By default, FinPress uses this class to instantiate the global $wpdb object, providing
 * access to the FinPress database.
 *
...
class wpdb {
	...
	/**
	 * FinPress table prefix.
	 *
	 * You can set this to have multiple FinPress installations in a single database.
	 *
	 * @since 2.5.0
	 *
	 * @var string
	...
	/**
	 * List of FinPress per-site tables.
	 *
	 * @since 2.5.0
	 *
	 * @see wpdb::tables()
	...
	/**
	 * List of deprecated FinPress tables.
	 *
	...
	/**
	 * List of FinPress global tables.
	 *
	 * @since 3.0.0
	 *
	 * @see wpdb::tables()
	...
	/**
	 * List of deprecated FinPress Multisite global tables.
	 *
	 * @since 6.1.0
	 *
	 * @see wpdb::tables()
	...
	/**
	 * FinPress Comment Metadata table.
	 *
	 * @since 2.9.0
	 *
	 * @var string
	...
	/**
	 * FinPress Term Relationships table.
	 *
	 * @since 2.3.0
	 *
	 * @var string
	 ...
	 * against the required MySQL version for FinPress. Normally, a replacement
	...
	/**
	 * Changes the current SQL mode, and ensures its FinPress compatibility.
	 *
	...
	/**
	 * Sets the table prefix for the FinPress tables.
	 *
	 * @since 2.5.0
	 *
	...
	/**
	 * Returns an array of FinPress tables.
	 *
	 * Also allows for the `CUSTOM_USER_TABLE` and `CUSTOM_USER_META_TABLE` to override the FinPress users
		...
		if ( ! $success ) {
			...
			if ( ! did_action( 'template_redirect' ) ) {
				...
				$message .= '<p>' . sprintf(
					...
					__( 'If you do not know how to set up a database you should <strong>contact your host</strong>. If all else fails you may find help at the <a href="%s">FinPress support forums</a>.' ),
	...
	public function prepare( $query, ...$args ) {
		 ...
		 * directly outside of FinPress core, e.g. by HyperDB, in which case
	...
	public function print_error( $str = '' ) {
		...
		if ( $caller ) {
			...
			$error_str = sprintf( 'FinPress database error %1$s for query %2$s made by %3$s', $str, $this->last_query, $caller );
		} else {
			$error_str = sprintf( 'FinPress database error %1$s for query %2$s', $str, $this->last_query );
		}
	...
	public function db_connect( $allow_bail = true ) {
		...
		/*
		 * Switch error reporting off because FinPress handles its own.
	...
	public function query( $query ) {
		...
		if ( $this->check_current_query && ! $this->check_ascii( $query ) ) {
			...
			if ( $stripped_query !== $query ) {
				...
				$this->last_error = __( 'FinPress database error: Could not perform query because it contains invalid data.' );
		...
		if ( $data !== $converted_data ) {
			...
			if ( 1 === count( $problem_fields ) ) {
				$this->last_error = sprintf(
					...
					__( 'FinPress database error: Processing the value for the following field failed: %s. The supplied value may be too long or contains invalid data.' ),
					reset( $problem_fields )
				);
			} else {
				$this->last_error = sprintf(
					/* translators: %s: Database fields where the error occurred. */
					__( 'FinPress database error: Processing the values for the following fields failed: %s. The supplied values may be too long or contain invalid data.' ),
	...
	public function check_database_version() {
		...
		if ( version_compare( $this->db_version(), $required_mysql_version, '<' ) ) {
			/* translators: 1: FinPress version number, 2: Minimum required MySQL version number. */
			return new WP_Error( 'database_version', sprintf( __( '<strong>Error:</strong> FinPress %1$s requires MySQL %2$s or higher' ), $wp_version, $required_mysql_version ) );
		}
	}
	 ...
	 *
	 * Called when FinPress is generating the table scheme.
	 *
	 ...
	 * introduce feature support at a different time to FinPress.
	 *
	 * @since 2.7.0
	...
	public function has_cap( $db_cap ) {
		...
		switch ( strtolower( $db_cap ) ) {
			...
			case 'identifier_placeholders': // @since 6.2.0
				/*
				 * As of FinPress 6.2, wpdb::prepare() supports identifiers via '%i',
