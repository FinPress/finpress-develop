<?php
/**
 * FinPress API for media display.
 *
 * @package FinPress
 * @subpackage Media
 */
 ...
 * parameters are: what FinPress will use for the class, the Attachment ID,
...
function wp_calculate_image_srcset( $size_array, $image_src, $image_meta, $attachment_id = 0 ) {
	...
	/*
	 * FinPress flattens animated GIFs into one frame when generating intermediate sizes.
	...
	/*
	 * Images that have been edited in FinPress after being uploaded will
...
function wp_img_tag_add_auto_sizes( string $image ): string {
	 ...
	 * Per FinPress Core itself, lazy-loaded images should always have a width attribute.
 ...
 * a collection of FinPress audio or video files in a post.
 *
 * @since 3.9.0
 *
 * @global int $content_width
...
function attachment_url_to_postid( $url ) {
	 ...
	 * - null to indicate FinPress should proceed with the lookup.
	 *
...
function wp_register_media_personal_data_exporter( $exporters ) {
	$exporters['finpress-media'] = array(
		'exporter_friendly_name' => __( 'FinPress Media' ),
 ...
 *
 * These sizes are meant to enhance the way FinPress displays images on the front-end on larger,
...
function wp_maybe_add_fetchpriority_high_attr( $loading_attrs, $tag_name, $attr ) {
	...
	if ( isset( $attr['fetchpriority'] ) ) {
		 ...
		 * is the only possible value that FinPress core would apply on its
		 * own.
		 */
