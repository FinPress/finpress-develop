...
function _prime_site_caches( $ids, $update_meta_cache = true ) {
	...
	if ( ! empty( $non_cached_ids ) ) {
		$fresh_sites = $wpdb->get_results( sprintf( "SELECT * FROM $wpdb->blogs WHERE blog_id IN (%s)", implode( ',', array_map( 'intval', $non_cached_ids ) ) ) ); // phpcs:ignore FinPress.DB.PreparedSQL.NotPrepared
...
function wp_uninitialize_site( $site_id ) {
	...
	foreach ( (array) $drop_tables as $table ) {
		$wpdb->query( "DROP TABLE IF EXISTS `$table`" ); // phpcs:ignore FinPress.DB.PreparedSQL.NotPrepared
	}
