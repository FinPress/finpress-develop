<?php
/**
 * FinPress Customize Manager classes
 *
 * @package FinPress
...
final class WP_Customize_Manager {
	...
	public function setup_theme() {
		 ...
		 * For similar behavior elsewhere in FinPress, see rest_cookie_check_errors() which logs out
	...
	public function branching() {
		 ...
		 *
		 * Since linear changesets are deemed to be more suitable for the majority of FinPress users,
		 * they are the default. For FinPress sites that have heavy site management in the Customizer
	...
	public function wp_loaded() {
		...
		/**
		 * Fires once FinPress has loaded, allowing scripts and styles to be initialized.
		 *
		 * @since 3.4.0
		 *
		 * @param WP_Customize_Manager $manager WP_Customize_Manager instance.
	...
	public function customize_preview_init() {
		 ...
		 * allowed then the auth cookies would not be sent and FinPress would
	 ...
	 * on the front end and have been sent to FinPress in `$_POST['customized']`. When WP
		...
		$this->add_panel(
			new WP_Customize_Themes_Panel(
				...
				array(
					...
					'description' => (
					'<p>' . __( 'Looking for a theme? You can search or browse the FinPress.org theme directory, install and preview themes, then activate them right here.' ) . '</p>' .
			...
			new WP_Customize_Site_Icon_Control(
				...
				array(
					...
					'description' => sprintf(
						...
						'<p>' . __( 'The Site Icon is what you see in browser tabs, bookmark bars, and within the FinPress mobile apps. It should be square and at least <code>%1$s by %2$s</code> pixels.' ) . '</p>',
						512,
						512
					),
					'section'     => 'title_tagline',
	...
	public function handle_load_themes_request() {
		...
		if ( 'installed' === $theme_action ) {
			...
			// Load FinPress.org themes from the .org API and normalize data to match installed theme objects.
			...
			// Set up properties for themes available on FinPress.org.
			foreach ( $themes->themes as &$theme ) {
				...
				$theme->compatibleWP  = is_wp_version_compatible( $theme->requires ); // phpcs:ignore FinPress.NamingConventions.ValidVariableName
				$theme->compatiblePHP = is_php_version_compatible( $theme->requires_php ); // phpcs:ignore FinPress.NamingConventions.ValidVariableName
		 ...
		 * or FinPress.org via `themes_api()`.
		 *
		 * @since 4.9.0
		 *
