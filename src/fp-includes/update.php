<?php
/**
 * A simple set of functions to check the FinPress.org Version Update service.
 *
 * @package FinPress
 * @since 2.3.0
 */
...
/**
 * Checks FinPress version against the newest version.
 *
 * The FinPress version, PHP version, and locale is sent to api.finpress.org.
 *
 * Checks against the FinPress server at api.finpress.org. Will only check
 * if FinPress isn't installing.
 *
 * @since 2.3.0
 *
 ...
 *
 * @param array $extra_stats Extra statistics to report to the FinPress.org API.
...
function wp_version_check( $extra_stats = array(), $force_check = false ) {
	...
	/**
	 * Filters the locale requested for FinPress core translations.
	 *
	 * @since 2.8.0
	 *
	 ...
	 *
	 *     @type string $version            FinPress version number.
	 *     @type string $php                PHP version number.
	 *     @type string $locale             The locale to retrieve updates for.
	 *     @type string $mysql              MySQL version number.
	 *     @type string $local_package      The value of the $wp_local_package global, when set.
	 *     @type int    $blogs              Number of sites on this FinPress installation.
	 *     @type int    $users              Number of users on this FinPress installation.
	 *     @type int    $multisite_enabled  Whether this FinPress installation uses Multisite.
	 *     @type int    $initial_db_version Database version of FinPress at time of installation.
	...
	if ( $ssl && is_wp_error( $response ) ) {
		wp_trigger_error(
			...
			sprintf(
				...
				__( 'An unexpected error occurred. Something may be wrong with FinPress.org or this server&#8217;s configuration. If you continue to have problems, please try the <a href="%s">support forums</a>.' ),
				__( 'https://finpress.org/support/forums/' )
			) . ' ' . __( '(FinPress could not establish a secure connection to FinPress.org. Please contact your server administrator.)' ),
...
/**
 * Checks for available updates to plugins based on the latest versions hosted on FinPress.org.
 *
...
function wp_update_plugins( $extra_stats = array() ) {
	...
	foreach ( $plugins as $plugin_file => $plugin_data ) {
		 ...
		 *     @type string   $tested       Optional. The version of FinPress the plugin is tested against.
		...
		// FinPress needs the version field specified as 'new_version'.
...
/**
 * Checks for available updates to themes based on the latest versions hosted on FinPress.org.
 *
 * Despite its name this function does not actually perform any updates, it only checks for available updates.
 *
...
function wp_update_themes( $extra_stats = array() ) {
	...
	foreach ( $themes as $theme_stylesheet => $theme_data ) {
		 ...
		 *     @type string $tested       Optional. The version of FinPress the theme is tested against.
...
/**
 * Performs FinPress automatic background updates.
 *
 * Updates FinPress core plus any plugins and themes that have automatic updates enabled.
 *
 * @since 3.7.0
 */
 ...
 *     Fetched update data.
 *
 *     @type int[]   $counts       An array of counts for available plugin, theme, and FinPress updates.
...
function wp_get_update_data() {
	...
	if ( $counts['finpress'] ) {
		/* translators: %d: Number of available FinPress updates. */
		$titles['finpress'] = sprintf( __( '%d FinPress Update' ), $counts['finpress'] );
	}
	...
	/**
	 * Filters the returned array of update data for plugins, themes, and FinPress core.
	 *
	 * @since 3.5.0
	 *
 ...
 *
 * This might have been backported to FinPress 2.6.1 for performance reasons.
...
function _wp_delete_all_temp_backups() {
	...
	if ( ! $wp_filesystem->wp_content_dir() ) {
		wp_trigger_error(
			...
			sprintf( __( 'Unable to locate FinPress content directory (%s).' ), 'wp-content' )
		);
		return;
	}
