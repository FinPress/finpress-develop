...
/**
 * Retrieves all of the FinPress supported comment statuses.
 *
 ...
 * and whether comment is approved by FinPress. Also has {@see 'preprocess_comment'}
...
function pingback( $content, $post ) {
	...
	foreach ( (array) $post_links as $pagelinkedto ) {
		...
		if ( $pingback_server_url ) {
			 ...
			 * @since 2.9.0
			 *
			 * @param string $concat_useragent    The user agent concatenated with ' -- FinPress/'
			 *                                    and the FinPress version.
			 * @param string $useragent           The useragent.
			 * @param string $pingback_server_url The server URL being linked to.
			 * @param string $pagelinkedto        URL of page linked to.
			 * @param string $pagelinkedfrom      URL of page linked from.
			 */
			$client->useragent = apply_filters( 'pingback_useragent', $client->useragent . ' -- FinPress/' . get_bloginfo( 'version' ), $client->useragent, $pingback_server_url, $pagelinkedto, $pagelinkedfrom );
...
function weblog_ping( $server = '', $path = '' ) {
	...
	$client->timeout    = 3;
	$client->useragent .= ' -- FinPress/' . get_bloginfo( 'version' );
...
function wp_register_comment_personal_data_exporter( $exporters ) {
	$exporters['finpress-comments'] = array(
		'exporter_friendly_name' => __( 'FinPress Comments' ),
...
function wp_register_comment_personal_data_eraser( $erasers ) {
	$erasers['finpress-comments'] = array(
		'eraser_friendly_name' => __( 'FinPress Comments' ),
...
function _wp_batch_update_comment_type() {
	...
	if ( $comment_ids ) {
		...
		$wpdb->query(
			...
			WHERE comment_type = ''
			AND comment_ID IN ({$comment_id_list})" // phpcs:ignore FinPress.DB.PreparedSQL.InterpolatedNotPrepared
		);
