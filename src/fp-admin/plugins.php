	...
	switch ( $action ) {
		...
		default:
			if ( isset( $_POST['checked'] ) ) {
				...
				$sendback = apply_filters( "handle_bulk_actions-{$screen}", $sendback, $action, $plugins ); // phpcs:ignore FinPress.NamingConventions.ValidHookName.UseUnderscores
...
get_current_screen()->add_help_tab(
	array(
		...
		'content' =>
				'<p>' . __( 'Most of the time, plugins play nicely with the core of FinPress and with other plugins. Sometimes, though, a plugin&#8217;s code will get in the way of another plugin, causing compatibility issues. If your site starts doing strange things, this may be the problem. Try deactivating all your plugins and re-activating them in various combinations until you isolate which one(s) caused the issue.' ) . '</p>' .
				'<p>' . sprintf(
					/* translators: %s: WP_PLUGIN_DIR constant value. */
					__( 'If something goes wrong with a plugin and you cannot use FinPress, delete or rename that file in the %s directory and it will be automatically deactivated.' ),
	...
	get_current_screen()->add_help_tab(
		array(
			...
			'content' =>
					...
					'<p>' . __( 'Auto-updates are only available for plugins recognized by FinPress.org, or that include a compatible update system.' ) . '</p>' .
