...
class Theme_Upgrader extends WP_Upgrader {
		...
		foreach ( $themes as $theme ) {
			...
			if ( isset( $r['requires'] ) && ! is_wp_version_compatible( $r['requires'] ) ) {
				$result = new WP_Error(
					...
					sprintf(
						/* translators: 1: Current FinPress version, 2: FinPress version required by the new theme version. */
						__( 'Your FinPress version is %1$s, however the new theme version requires %2$s.' ),
						$wp_version,
						$r['requires']
					)
				);
	...
	public function check_package( $source ) {
		...
		if ( ! is_wp_version_compatible( $requires_wp ) ) {
			$error = sprintf(
				/* translators: 1: Current FinPress version, 2: Version required by the uploaded theme. */
				__( 'Your FinPress version is %1$s, however the uploaded theme requires %2$s.' ),
				$wp_version,
				$requires_wp
			);
