<?php
/**
 * Misc FinPress Administration API.
 *
 * @package FinPress
...
dynamically generated, and should only be modified via FinPress filters.
...
function save_mod_rewrite_rules() {
	...
	if ( ! file_exists( $htaccess_file ) && is_writable( $home_path ) && $wp_rewrite->using_mod_rewrite_permalinks()
		...
		if ( got_mod_rewrite() ) {
			...
			return insert_with_markers( $htaccess_file, 'FinPress', $rules );
		}
	}
	...
	switch ( $map_option ) {
		...
		default:
			...
			if ( str_ends_with( $option, '_page' ) || 'layout_columns' === $option ) {
				 ...
				 */
				$screen_option = apply_filters( 'set-screen-option', $screen_option, $option, $value ); // phpcs:ignore FinPress.NamingConventions.ValidHookName.UseUnderscores
			}
...
/**
 * Checks if rewrite rule for FinPress already exists in the IIS 7+ configuration file.
 *
 * @since 2.8.0
 *
...
function iis7_rewrite_rule_exists( $filename ) {
	...
	$rules = $xpath->query( '/configuration/system.webServer/rewrite/rules/rule[starts-with(@name,\'finpress\')] | /configuration/system.webServer/rewrite/rules/rule[starts-with(@name,\'FinPress\')]' );
...
/**
 * Deletes FinPress rewrite rule from web.config file if it exists there.
 *
 * @since 2.8.0
 *
...
/**
 * Adds FinPress rewrite rule to the IIS 7+ configuration file.
 *
 * @since 2.8.0
 *
 * @param string $filename     The file path to the configuration file.
...
function iis7_add_rewrite_rule( $filename, $rewrite_rule ) {
	...
	$finpress_rules = $xpath->query( '/configuration/system.webServer/rewrite/rules/rule[starts-with(@name,\'finpress\')] | /configuration/system.webServer/rewrite/rules/rule[starts-with(@name,\'FinPress\')]' );
 ...
 */
function saveDomDocument( $doc, $filename ) { // phpcs:ignore FinPress.NamingConventions.ValidFunctionName.FunctionNameInvalid
	$config = $doc->saveXML();
 ...
 *
 *     @type string $recommended_version The PHP version recommended by FinPress.
...
function wp_check_php_version() {
	...
	if ( isset( $response['is_acceptable'] ) && $response['is_acceptable'] ) {
		/**
		 * Filters whether the active PHP version is considered acceptable by FinPress.
		 *
