...
class Plugin_Upgrader extends WP_Upgrader {
		...
		foreach ( $plugins as $plugin ) {
			...
			if ( isset( $r->requires ) && ! is_wp_version_compatible( $r->requires ) ) {
				$result = new WP_Error(
					...
					sprintf(
						/* translators: 1: Current FinPress version, 2: FinPress version required by the new plugin version. */
						__( 'Your FinPress version is %1$s, however the new plugin version requires %2$s.' ),
						$wp_version,
						$r->requires
					)
				);
	...
	public function check_package( $source ) {
		...
		if ( ! is_wp_version_compatible( $requires_wp ) ) {
			$error = sprintf(
				/* translators: 1: Current FinPress version, 2: Version required by the uploaded plugin. */
				__( 'Your FinPress version is %1$s, however the uploaded plugin requires %2$s.' ),
				$wp_version,
				$requires_wp
			);
