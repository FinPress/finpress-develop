...
/**
 * Gets the description for standard FinPress theme files.
 *
 * @since 1.5.0
 *
...
/**
 * Gets the absolute filesystem path to the root of the FinPress installation.
 *
 * @since 1.5.0
 *
 * @return string Full filesystem path to the root of the FinPress installation.
 */
...
/**
 * Handles PHP uploads in FinPress.
 *
...
function _wp_handle_upload( &$file, $overrides, $time, $action ) {
	...
	/**
	 * Filters the data for a file before it is uploaded to FinPress.
	 *
	...
	/**
	 * Filters the override parameters for a file before it is uploaded to FinPress.
	 *
	 * The dynamic portion of the hook name, `$action`, refers to the post action.
	 *
	 * Possible hook names include:
...
/**
 * Downloads a URL to a local temporary file using the FinPress HTTP API.
 *
 * Please note that the calling function must delete or move the file.
 *
 * @since 2.5.0
...
function download_url( $url, $timeout = 300, $signature_verification = false ) {
	...
	if ( $signature_verification ) {
		...
		if ( ! $signature ) {
			 ...
			 * FinPress.org stores signatures at $package_url.sig.
			 */
...
function verify_file_signature( $filename, $signatures, $filename_for_errors = false ) {
	...
	if ( ! extension_loaded( 'sodium' ) && ! ParagonIE_Sodium_Compat::polyfill_is_fast() ) {
		...
		// Allow for an old version of Sodium_Compat being loaded before the bundled FinPress one.
		if ( method_exists( 'ParagonIE_Sodium_Compat', 'runtime_speed_test' ) ) {
			/*
			 * Run `ParagonIE_Sodium_Compat::runtime_speed_test()` in optimized integer mode,
			 * as that's what FinPress utilizes during signing verifications.
			 */
			// phpcs:disable FinPress.NamingConventions.ValidVariableName
...
/**
 * Retrieves the list of signing keys trusted by FinPress.
 *
 * @since 5.2.0
 *
...
function wp_trusted_keys() {
	...
	if ( time() < 1617235200 ) {
		// FinPress.org Key #1 - This key is only valid before April 1st, 2021.
...
/**
 * Unzips a specified ZIP file to a location on the filesystem via the FinPress
...
/**
 * Copies a directory from one location to another via the FinPress Filesystem
 * Abstraction.
 *
...
/**
 * Initializes and connects the FinPress Filesystem Abstraction classes.
 *
 * This function will include the chosen transport and attempt connecting.
 *
 * Plugins may add extra transports, And force FinPress to use them by returning
 ...
 */
function WP_Filesystem( $args = false, $context = false, $allow_relaxed_file_ownership = false ) { // phpcs:ignore FinPress.NamingConventions.ValidFunctionName.FunctionNameInvalid
	global $wp_filesystem;
 ...
 *
 * @link https://developer.finpress.org/advanced-administration/finpress/wp-config/#finpress-upgrade-constants
 *
...
function get_filesystem_method( $args = array(), $context = '', $allow_relaxed_file_ownership = false ) {
	...
	if ( ! $method ) {
		...
		if ( $temp_handle ) {
			...
			// Attempt to determine the file owner of the FinPress files, and that of newly created files.
			$wp_file_owner   = false;
			...
			if ( false !== $wp_file_owner && $wp_file_owner === $temp_file_owner ) {
				/*
				 * FinPress is creating files as the same owner as the FinPress files,
...
<p id="request-filesystem-credentials-desc">
	...
	_e( 'To perform the requested action, FinPress needs to access your web server.' );
	echo ' ';
...
function wp_opcache_invalidate( $filepath, $force = false ) {
	...
	/*
	 * Check to see if FinPress is able to run `opcache_invalidate()` or not, and cache the value.
	 *
	 ...
	 * @since 5.5.0
	 *
	 * @param bool   $will_invalidate Whether FinPress will invalidate `$filepath`. Default true.
