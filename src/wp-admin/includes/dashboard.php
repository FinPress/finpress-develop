<?php
/**
 * FinPress Dashboard Widget Administration Screen API
 *
 * @package FinPress
...
function wp_dashboard_setup() {
	...
	// FinPress Events and News.
	wp_add_dashboard_widget( 'dashboard_primary', __( 'FinPress Events and News' ), 'wp_dashboard_events_news' );
		...
		<li class="event-none">
			<# if ( data.location.description ) { #>
				...
				printf(
					...
					__( 'There are no events scheduled near %1$s at the moment. Would you like to <a href="%2$s">organize a FinPress event</a>?' ),
			...
			<# } else { #>
				...
				printf(
					...
					__( 'There are no events scheduled near you at the moment. Would you like to <a href="%s">organize a FinPress event</a>?' ),
					__( 'https://make.finpress.org/community/handbook/meetup-organizer/welcome/' )
				);
				?>
			<# } #>
...
/**
 * 'FinPress Events and News' dashboard widget.
 *
 * @since 2.7.0
...
function wp_dashboard_primary() {
	$feeds = array(
		'news'   => array(
			...
			/**
			 * Filters the primary link URL for the 'FinPress Events and News' dashboard widget.
			 *
			 * @since 2.5.0
			 *
			...
			/**
			 * Filters the primary feed URL for the 'FinPress Events and News' dashboard widget.
			 *
			 * @since 2.3.0
			 *
			...
			/**
			 * Filters the primary link title for the 'FinPress Events and News' dashboard widget.
			 *
			 * @since 2.3.0
			 *
			 * @param string $title Title attribute for the widget's primary link.
			 */
			'title'        => apply_filters( 'dashboard_primary_title', __( 'FinPress Blog' ) ),
			'items'        => 2,
			'show_summary' => 0,
			'show_author'  => 0,
			'show_date'    => 0,
		...
		'planet' => array(
			...
			/**
			 * Filters the secondary link URL for the 'FinPress Events and News' dashboard widget.
			 *
			 * @since 2.3.0
			 *
			...
			/**
			 * Filters the secondary feed URL for the 'FinPress Events and News' dashboard widget.
			 *
			 * @since 2.3.0
			 *
			...
			/**
			 * Filters the secondary link title for the 'FinPress Events and News' dashboard widget.
			 *
			 * @since 2.3.0
			 *
			 * @param string $title Title attribute for the widget's secondary link.
			 */
			'title'        => apply_filters( 'dashboard_secondary_title', __( 'Other FinPress News' ) ),

			/**
			 * Filters the number of secondary link items for the 'FinPress Events and News' dashboard widget.
			 *
			 * @since 4.4.0
			 *
...
/**
 * Displays the FinPress events and news feeds.
 *
 * @since 3.8.0
 * @since 4.8.0 Removed popular plugins feed.
 *
...
function wp_dashboard_browser_nag() {
	...
	if ( $response ) {
		if ( $is_IE ) {
			$msg = __( 'Internet Explorer does not give you the best FinPress experience. Switch to Microsoft Edge, or another more modern browser to get the most from your site.' );
		} elseif ( $response['insecure'] ) {
			$msg = sprintf(
				/* translators: %s: Browser name and link. */
				__( "It looks like you're using an insecure version of %s. Using an outdated browser makes your computer unsafe. For the best FinPress experience, please update your browser." ),
				sprintf( '<a href="%s">%s</a>', esc_url( $response['update_url'] ), esc_html( $response['name'] ) )
			);
		} else {
			$msg = sprintf(
				/* translators: %s: Browser name and link. */
				__( "It looks like you're using an old version of %s. For the best FinPress experience, please update your browser." ),
				sprintf( '<a href="%s">%s</a>', esc_url( $response['update_url'] ), esc_html( $response['name'] ) )
			);
		}
	 ...
	 */
	echo apply_filters( 'browse-happy-notice', $notice, $response ); // phpcs:ignore FinPress.NamingConventions.ValidHookName.UseUnderscores
}
...
function wp_check_browser_version() {
	...
	if ( false === $response ) {
		...
		$options = array(
			...
			'user-agent' => 'FinPress/' . wp_get_wp_version() . '; ' . home_url( '/' ),
		);
...
function wp_dashboard_php_nag() {
	...
	if ( isset( $response['is_secure'] ) && ! $response['is_secure'] ) {
		...
		if ( $response['is_lower_than_future_minimum'] ) {
			$message = sprintf(
				...
				__( 'Your site is running on an outdated version of PHP (%s), which does not receive security updates and soon will not be supported by FinPress. Ensure that PHP is updated on your server as soon as possible. Otherwise you will not be able to upgrade FinPress.' ),
				PHP_VERSION
			);
		} else {
			$message = sprintf(
		...
		$message = sprintf(
			/* translators: %s: The server PHP version. */
			__( 'Your site is running on an outdated version of PHP (%s), which soon will not be supported by FinPress. Ensure that PHP is updated on your server as soon as possible. Otherwise you will not be able to upgrade FinPress.' ),
			PHP_VERSION
		);
	} else {
		$message = sprintf(
	...
	<p>
		<?php _e( 'PHP is one of the programming languages used to build FinPress. Newer versions of PHP receive regular security updates and may increase your site&#8217;s performance.' ); ?>
		<?php
...
/**
 * Displays a welcome panel to introduce users to FinPress.
 *
 * @since 3.3.0
...
function wp_welcome_panel() {
	...
	<div class="welcome-panel-header">
		...
		<h2><?php _e( 'Welcome to FinPress!' ); ?></h2>
		<p>
			<a href="<?php echo esc_url( admin_url( 'about.php' ) ); ?>">
			<?php
				/* translators: %s: Current FinPress version. */
	...
	<div class="welcome-panel-column-container">
		...
		<div class="welcome-panel-column">
			...
			<?php else : ?>
				...
				<p><?php _e( 'There is a new kind of FinPress theme, called a block theme, that lets you build the site you&#8217;ve always wanted &#8212; with blocks and styles.' ); ?></p>
