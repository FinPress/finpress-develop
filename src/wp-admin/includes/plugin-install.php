<?php
/**
 * FinPress Plugin Install Administration API
 *
 * @package FinPress
...
/**
 * Retrieves plugin installer pages from the FinPress.org Plugins API.
 *
 ...
 *
 * The second filter, {@see 'plugins_api'}, allows a plugin to override the FinPress.org
 ...
 *         @type bool $requires          Whether to return the required FinPress version. Default true.
 ...
 *         @type bool $compatibility     Whether to return the FinPress compatibility list. Default true.
...
function plugins_api( $action, $args = array() ) {
	...
	/**
	 * Filters the FinPress.org Plugin Installation API arguments.
	 *
	...
	/**
	 * Filters the response for the current FinPress.org Plugin Installation API request.
	 *
	 * Returning a non-false value will effectively short-circuit the FinPress.org API request.
	 *
...
/**
 * Retrieves popular FinPress plugin tags.
 *
 * @since 2.7.0
 *
 * @param array $args
...
function install_plugins_favorites_form() {
	...
	?>
	<p><?php _e( 'If you have marked plugins as favorites on FinPress.org, you can browse them here.' ); ?></p>
	<form method="get">
		<input type="hidden" name="tab" value="favorites" />
		<p>
			<label for="user"><?php _e( 'Your FinPress.org username:' ); ?></label>
	...
	switch ( current_filter() ) {
		case 'install_plugins_beta':
			printf(
				...
				'<p>' . __( 'You are using a development version of FinPress. These feature plugins are also under development. <a href="%s">Learn more</a>.' ) . '</p>',
...
<div id="<?php echo $_tab; ?>-content" class='<?php echo $_with_banner; ?>'>
	<div class="fyi">
		<ul>
			...
			<?php } if ( ! empty( $api->requires ) ) { ?>
				<li>
					<strong><?php _e( 'Requires FinPress Version:' ); ?></strong>
					<?php
			...
			<?php } if ( ! empty( $api->slug ) && empty( $api->external ) ) { ?>
				<li><a target="_blank" href="<?php echo esc_url( __( 'https://finpress.org/plugins/' ) . $api->slug ); ?>/"><?php _e( 'FinPress.org Plugin Page &#187;' ); ?></a></li>
		...
		if ( ! empty( $api->ratings ) && array_sum( (array) $api->ratings ) > 0 ) {
			...
			<p class="fyi-description"><?php _e( 'Read all reviews on FinPress.org or write your own!' ); ?></p>
			<?php
	...
	if ( ! $tested_wp ) {
		wp_admin_notice(
			__( '<strong>Warning:</strong> This plugin <strong>has not been tested</strong> with your current version of FinPress.' ),
			array(
				'type'               => 'warning',
				'additional_classes' => array( 'notice-alt' ),
			)
		...
		$compatible_wp_notice_message = __( '<strong>Error:</strong> This plugin <strong>requires a newer version of FinPress</strong>.' );
		if ( current_user_can( 'update_core' ) ) {
			$compatible_wp_notice_message .= sprintf(
				/* translators: %s: URL to FinPress Updates screen. */
				' ' . __( '<a href="%s" target="_parent">Click here to update FinPress</a>.' ),
