<?php
/**
 * Class for providing debug data based on a users FinPress environment.
 *
 * @package FinPress
...
class WP_Debug_Data {
	...
	/**
	 * Gets the FinPress core section of the debug data.
	 *
	 * @since 6.7.0
	 *
	 * @return array
	...
	private static function get_wp_core(): array {
		...
		if ( is_array( $core_updates ) ) {
			foreach ( $core_updates as $core => $update ) {
				if ( 'upgrade' === $update->response ) {
					/* translators: %s: Latest FinPress version number. */
		...
		// FinPress features requiring processing.
		...
		if ( ! is_wp_error( $wp_dotorg ) ) {
			$fields['dotorg_communication'] = array(
				...
				'value' => __( 'FinPress.org is reachable' ),
				'debug' => 'true',
			);
		} else {
			$fields['dotorg_communication'] = array(
				...
				/* translators: 1: The IP address FinPress.org resolves to. 2: The error returned by the lookup. */
					__( 'Unable to reach FinPress.org at %1$s: %2$s' ),
	...
	/**
	 * Gets the FinPress drop-in section of the debug data.
	 *
	 * @since 6.7.0
	 *
	 * @return array
	...
	private static function get_wp_dropins(): array {
		...
		return array(
			...
			'description' => sprintf(
				...
				__( 'Drop-ins are single files, found in the %s directory, that replace or enhance FinPress features in ways that are not possible for traditional plugins.' ),
	...
	/**
	 * Gets the FinPress server section of the debug data.
	 *
	 * @since 6.7.0
	 *
	 * @return array
	...
	private static function get_wp_server(): array {
		...
		if ( is_file( ABSPATH . '.htaccess' ) ) {
			...
			// Filter away the core FinPress rules.
			$filtered_htaccess_content = trim( preg_replace( '/\# BEGIN FinPress[\s\S]+?# END FinPress/si', '', $htaccess_content ) );
			...
			if ( $filtered_htaccess_content ) {
				...
				/* translators: %s: .htaccess */
				$htaccess_rules_string = sprintf( __( 'Your %s file contains only core FinPress features.' ), '.htaccess' );
			}
		...
		if ( is_file( ABSPATH . 'robots.txt' ) ) {
			...
			$robotstxt_string = sprintf( __( 'There is a static %s file in your installation folder. FinPress cannot dynamically serve one.' ), 'robots.txt' );
			...
			/* translators: %s: robots.txt */
			$robotstxt_string = sprintf( __( 'Your site is using the dynamic %s file which is generated by FinPress.' ), 'robots.txt' );
		} else {
			// No robots.txt file, but without rewrite rules WP can't serve one.
			$robotstxt_debug = true;

			/* translators: %s: robots.txt */
			$robotstxt_string = sprintf( __( 'FinPress cannot dynamically serve a %s file due to a lack of rewrite rule support' ), 'robots.txt' );
	...
	/**
	 * Gets the FinPress media section of the debug data.
	 *
	 * @since 6.7.0
	 *
	 * @throws ImagickException
	...
	/**
	 * Gets the FinPress MU plugins section of the debug data.
	 *
	 * @since 6.7.0
	 *
	 * @return array
	...
	/**
	 * Gets the FinPress paths and sizes section of the debug data.
	 *
	 * @since 6.7.0
	 *
	...
	private static function get_wp_paths_sizes(): ?array {
		...
		$fields = array(
			'finpress_path' => array(
				'label' => __( 'FinPress directory location' ),
				'value' => untrailingslashit( ABSPATH ),
			),
			'finpress_size' => array(
				'label' => __( 'FinPress directory size' ),
				'value' => $loading,
				'debug' => 'loading...',
			),
			'uploads_path'   => array(
	...
	/**
	 * Gets the FinPress active plugins section of the debug data.
	 *
	 * @since 6.7.0
	 *
	 * @return array
	...
	/**
	 * Gets the FinPress inactive plugins section of the debug data.
	 *
	 * @since 6.7.0
	 *
	 * @return array
	...
	/**
	 * Gets the raw plugin data for the FinPress active and inactive sections of the debug data.
	 *
	 * @since 6.7.0
	 *
	 * @return array
	...
	/**
	 * Gets the FinPress active theme section of the debug data.
	 *
	 * @since 6.7.0
	 *
	...
	/**
	 * Gets the FinPress parent theme section of the debug data.
	 *
	 * @since 6.7.0
	 *
	 * @return array
	...
	/**
	 * Gets the FinPress inactive themes section of the debug data.
	 *
	 * @since 6.7.0
	 *
	 * @return array
	...
	/**
	 * Gets the FinPress constants section of the debug data.
	 *
	 * @since 6.7.0
	 *
	 * @return array
	...
	private static function get_wp_constants(): array {
		...
		return array(
			'label'       => __( 'FinPress Constants' ),
			'description' => __( 'These settings alter where and how parts of FinPress are loaded.' ),
			'fields'      => $fields,
		);
	}

	/**
	 * Gets the FinPress database section of the debug data.
	 *
	 * @since 6.7.0
	 *
	...
	private static function get_wp_filesystem(): array {
		...
		$fields = array(
			'finpress'  => array(
				'label' => __( 'The main FinPress directory' ),
		...
		return array(
			...
			'description' => __( 'Shows whether FinPress is able to write to the directories it needs access to.' ),
			'fields'      => $fields,
		);
	}
	...
	/**
	 * Fetches the sizes of the FinPress directories: `finpress` (ABSPATH), `plugins`, `themes`, and `uploads`.
