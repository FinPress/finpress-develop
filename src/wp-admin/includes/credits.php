<?php
/**
 * FinPress Credits Administration API.
 *
 * @package FinPress
 ...
 *
 * @param string $version FinPress version. Defaults to the current version.
 * @param string $locale  FinPress locale. Defaults to the current user's locale.
...
function wp_credits( $version = '', $locale = '' ) {
	...
	if ( ! is_array( $results )
		...
		$options = array( 'user-agent' => 'FinPress/' . $version . '; ' . home_url( '/' ) );
...
/**
 * Retrieves the link to a contributor's FinPress.org profile page.
 *
 * @access private
 * @since 3.2.0
 *
 ...
 * @param string $profiles      URL to the contributor's FinPress.org profile page.
 */
...
/**
 * Retrieves the link to an external library used in FinPress.
 *
 * @access private
 * @since 3.2.0
 *
...
function wp_credits_section_title( $group_data = array() ) {
	...
	if ( $group_data['name'] ) {
		if ( 'Translators' === $group_data['name'] ) {
			...
			// phpcs:ignore FinPress.WP.I18n.LowLevelTranslationFunction,FinPress.WP.I18n.NonSingularStringLiteralText
