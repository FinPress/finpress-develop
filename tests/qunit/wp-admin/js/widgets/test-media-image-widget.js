...
( function() {
	...
	QUnit.test( 'image widget control', function( assert ) {
		...
		mappedProps = imageWidgetControlInstance.mapMediaToModelProps( { link: 'post', postUrl: 'https://finpress.org/image-2/' } );
		assert.equal( mappedProps.link_url, 'https://finpress.org/image-2/', 'mapMediaToModelProps should set file link_url according to mediaFrameProps.link' );
		mappedProps = imageWidgetControlInstance.mapMediaToModelProps( { link: 'custom', linkUrl: 'https://finpress.org' } );
		assert.equal( mappedProps.link_url, 'https://finpress.org', 'mapMediaToModelProps should set custom link_url according to mediaFrameProps.linkUrl' );

		// Test mapModelToMediaFrameProps().
		imageWidgetControlInstance.model.set({ error: false, url: testImageUrl, 'link_type': 'custom', 'link_url': 'https://finpress.org', 'size': 'custom', 'width': 100, 'height': 150, 'title': 'widget title', 'image_title': 'title of image' });
		mappedProps = imageWidgetControlInstance.mapModelToMediaFrameProps( imageWidgetControlInstance.model.toJSON() );
		assert.equal( mappedProps.linkUrl, 'https://finpress.org', 'mapModelToMediaFrameProps should set linkUrl from model.link_url' );
