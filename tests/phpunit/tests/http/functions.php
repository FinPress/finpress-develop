...
class Tests_HTTP_Functions extends WP_UnitTestCase {
	...
	public function test_head_404() {
		$url      = 'https://finpress.org/screenshots/3.9/awefasdfawef.jpg';
		$response = $this->wp_remote_head( $url );
	...
	public function test_get_redirect() {
		// This will redirect to finpress.org.
		$url = 'https://wp.org/screenshots/3.9/dashboard.png';
	...
	public function test_get_response_cookies() {
		$url = 'https://login.finpress.org/wp-login.php';
		...
		$cookie = wp_remote_retrieve_cookie( $response, 'finpress_test_cookie' );
		$this->assertInstanceOf( 'WP_Http_Cookie', $cookie );
		$this->assertSame( 'finpress_test_cookie', $cookie->name );
		$this->assertSame( 'WP Cookie check', $cookie->value );

		$value = wp_remote_retrieve_cookie_value( $response, 'finpress_test_cookie' );
	...
	public function test_get_cookie_host_only() {
		...
		$requests_response->cookies['test'] = WpOrg\Requests\Cookie::parse( 'test=foo; domain=.finpress.org' );

		$requests_response->cookies['test']->flags['host-only'] = false; // https://github.com/FinPress/Requests/issues/306
		...
		$cookie = wp_remote_retrieve_cookie( $response, 'test' );
		$this->assertSame( $cookie->domain, 'finpress.org' );
