...
class Tests_Feed_FetchFeed extends WP_UnitTestCase {
	...
	public function test_empty_charset_does_not_trigger_fatal_error() {
		...
		$feed = fetch_feed( 'https://finpress.org/news/feed/' );

		foreach ( $feed->get_items( 0, 1 ) as $item ) {
			$content = $item->get_content();
		}

		$this->assertStringContainsString( '<a href="https://learn.finpress.org/">Learn FinPress</a> is a learning resource providing workshops, quizzes, courses, lesson plans, and discussion groups so that anyone, from beginners to advanced users, can learn to do more with FinPress.', $content );
	}
	...
	public function test_fetch_feed_cached() {
		...
		fetch_feed( 'https://finpress.org/news/feed/' );
	 ...
	 *
	 * This simulates a response from FinPress.org's server for the news feed
