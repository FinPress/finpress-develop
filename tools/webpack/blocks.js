...
module.exports = function (
	...
	const blockMetadataCopies = Object.keys( blockMetadataFiles ).map(
		( filename ) => ( {
			from: normalizeJoin(
				baseDir,
				`node_modules/@finpress/${ filename }`
			),
			to: normalizeJoin(
				baseDir,
	...
	const blockStylesheetCopies = blockFolders.map( ( blockName ) => ( {
		from: normalizeJoin(
			baseDir,
			`node_modules/@finpress/block-library/build-style/${ blockName }/*.css`
		),
		to: normalizeJoin(
			baseDir,
	...
	const config = {
		...
		entry: interactiveBlocks.reduce(( memo, blockName ) => {
			memo[ blockName ] = {
				import: normalizeJoin(
					baseDir,
					`node_modules/@finpress/block-library/build-module/${ blockName }/view`
				),
			};
			return memo;
		}, {}),
		...
		externals: {
			'@finpress/interactivity': '@finpress/interactivity',
			'@finpress/interactivity-router': 'import @finpress/interactivity-router',
		},
		plugins: [
			...baseConfig.plugins,
